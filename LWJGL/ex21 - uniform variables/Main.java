class Main{
	...
	public Main(){
		...
		shader = new Shader();
		String vs_source = ResourceManager.loadShader("vertex_shader.GLSL");
		String fs_source = ResourceManager.loadShader("fragment_shader.GLSL");
		
		shader.addVertexShader(vs_source);
		shader.addFragmentShader(fs_source);
		shader.compileShader();
		shader.addUniform("snopp");
		/**Temporary code END*/
	}
	...
	private void run(){
		...
		while( isRunning ){
			...
			if( ms_since_last_render > (FRAME_TIMECAP -(LOOP_TIMECAP/2)) ){ //-(LOOP_TIMECAP/2) allows for error margin
				shader.setUniformf("snopp", TEMP___________);
